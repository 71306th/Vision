...
public class Swerve extends SubsystemBase {
  ...
  public double calculateAutoFacing() {
    if((Constants.Vision.id == 4 || Constants.Vision.id == 7) && canCalculate == 9){
      for(int i=0;i<10;i++){
    ...
    }else if(Constants.Vision.id == 4 || Constants.Vision.id == 7){
      visionYawValue[canCalculate] = Storage.VisionStorage.VisionYaw;
      canCalculate++;
      RotationVal = lastRotationVal;
    }
    return -RotationVal;
    // } else if (Constants.Vision.id == 3 || Constants.Vision.id == 8) {
    //   double IDToSpeaker = Math.sqrt(Math.pow(Constants.Vision.RightIDToCenterY, 2) + Math.pow(Constants.Vision.CenterIDToSpeakerZ, 2)); // x
    //   double BotToTag = Storage.VisionStorage.VisionPose.getZ() / Math.cos(Constants.Vision.CameraRoll) / Math.cos(Storage.VisionStorage.VisionYaw); // y
  ...
  public void periodic() {
    ...
    SmartDashboard.putBoolean("isAuto", Constants.SuperStructure.isAuto);
