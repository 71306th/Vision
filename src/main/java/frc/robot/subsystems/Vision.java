...
public class Vision extends SubsystemBase {
  ...
  public void periodic() {
    ...
    y = ty.get();
    area = ta.get();
    Constants.Vision.id = tid.get();
    coordinateArr = coordinate.get();

    Storage.VisionStorage.VisionPose = new Pose3d(new Translation3d(coordinateArr[0], coordinateArr[1], coordinateArr[2]), new Rotation3d(coordinateArr[5], coordinateArr[3], coordinateArr[4]));
    Storage.VisionStorage.VisionYaw = x;

    SmartDashboard.putNumber("LimelightVaild", valid);
    SmartDashboard.putNumber("LimelightX", x);
    SmartDashboard.putNumber("LimelightY", y);
    SmartDashboard.putNumber("LimelightArea", area);
    SmartDashboard.putNumber("LimelightID", Constants.Vision.id);
